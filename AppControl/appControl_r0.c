...
//                            Turning socket_r4 into a library and using its function in appControl_r1.c
...
int main ( void )
{
   ...
   // Set North magnitude
   north = maxNorth - (maxNorth/8); //2700;//3100;
...
//         printf("COMMAND ARRAY: A %d, M %d\n", cmdArray[0], cmdArray[1]);
   ...
   //      speed += calculateSpeed( speed, cmdArray[1] );
         ...
         forwardBackwardM1( E_Stop*( stop + speed1 +  turn ), ser4 );
         forwardBackwardM2( E_Stop*( stop + speed2 + -turn ), ser4 );    
         i = 0;
         }
         i++;
         rcvcnt = 0;
