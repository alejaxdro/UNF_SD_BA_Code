...
int calculateSpeed( int topspeed, int speed, int turn, int cmdMagn ) {
   ...
   cmdSpeed = cmdMagn * (((topspeed-20)*1000) / 300);
   cmdSpeed = cmdSpeed / 1000;
   
   printf("TopS: %2d, CurS: %3d, CurTurn: %2d, cmdM: %3d, cmdS: %2d\n", topspeed, speed, turn, cmdMagn, cmdSpeed);
   if( speedEnable == 1 ){
      if( speed < (cmdSpeed - 5) ){
         return dspeed;
      }else if( (speed < cmdSpeed) && (speed >= (cmdSpeed - 5)) ){
         return -dspeed;
      }else{
         return -speed;
      }
...
int calculateTurnRate( int maxNorth, int turn, int cmdAngle ) {
   ...
   if( controlAngle > 190 ){
      ...
      // Decelerate when within range
//      if( controlAngle <= 220 && controlAngle > 190 ){
//         return dspeed;
//      }else if( controlAngle >= 140 && controlAngle > 170 ){
//         return -dspeed;
//      }else{
         speedEnable = 0;
         return -turn;
